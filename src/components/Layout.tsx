...
import {ChevronLeft, Home, LayoutGrid, Map as MapIcon, User, UserCircle} from 'lucide-react';
    ...
    <div className="min-h-screen flex flex-col max-w-md mx-auto bg-background relative overflow-x-hidden">
      <header className="fixed top-0 w-full max-w-md h-16 z-50 flex justify-between items-center px-6 py-4 bg-background/80 backdrop-blur-md">
        <div className="flex items-center gap-2">
          {showBack ? (
            <button onClick={onBack} className="p-1 active:scale-95 transition-transform" type="button">
        ...
        <div className="flex items-center gap-4">
          <button className="p-1 active:scale-95 transition-transform" type="button" onClick={() => setActiveTab('profile')}>
      ...
      <nav className="fixed bottom-0 w-full max-w-md z-50 flex justify-around items-center px-4 pb-6 pt-2 bg-[#99daff]/80 backdrop-blur-xl rounded-t-[3rem] shadow-[0_-4px_32px_rgba(72,39,2,0.06)]">
        <NavItem icon={<Home className="w-6 h-6" />} label="首页" active={activeTab === 'home'} onClick={() => setActiveTab('home')} />
        <NavItem icon={<MapIcon className="w-6 h-6" />} label="地图" active={activeTab === 'map'} onClick={() => setActiveTab('map')} />
        <NavItem
          ...
          isCenter
        />
        <NavItem icon={<User className="w-6 h-6" />} label="我的" active={activeTab === 'profile'} onClick={() => setActiveTab('profile')} />
      </nav>
    </div>
  );
};
